...
class MangaBakaApi(
    ...
    private val client = baseClient.newBuilder().addInterceptor {
        it.request().newBuilder()
            .header(
                "User-Agent",
                buildString {
                    append("Houri/v${BuildConfig.VERSION_NAME} ")
                    append("(${BuildConfig.APPLICATION_ID} ${BuildConfig.COMMIT_SHA}) ")
                    append("(Android) (https://github.com/PineappleTwilight/komikku-pineapple)")
                },
            )
            .build()
            .let(it::proceed)
        ...
        return withIOContext {
            ...
            val body = buildJsonObject {
                ...
                if (track.finished_reading_date > 0) {
                    put("finish_date", epochMillisToLocalDate(track.finished_reading_date))
                }
            }
                .toString()
                .toRequestBody()
    ...
    companion object {
        ...
        private const val REDIRECT_URI = "houri://mangabaka-auth"
    ...
    private fun epochMillisToLocalDate(epochMillis: Long): String {
